...
// position.
void Haptic_Driver::_writeRegister(uint8_t _wReg, uint8_t _mask, uint8_t _bits, uint8_t _startPosition)
{
...
// given register. 
uint8_t Haptic_Driver::_readConsReg(uint8_t _reg, uint8_t _numReads){
}
...
uint8_t Haptic_Driver::_readNonConsReg(uint8_t _reg){
}
...
uint8_t Haptic_Driver::_writeConsReg(uint8_t* regs){
  ...
  static const size_t numReads = sizeof(regs)/sizeof(*regs); 
  _i2cPort->beginTransmission(regs[i]);
...
// This particular write does not care what is currently in the register and
// overwrites whatever is there.
uint8_t Haptic_Driver::_writeNonConsReg(uint8_t* _reg){
